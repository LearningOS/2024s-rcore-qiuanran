...
use crate::{
    ...
    mm::{MapPermission, VirtAddr, translated_refmut, translated_str},
    task::{
...
pub fn sys_spawn(_path: *const u8) -> isize {
    ...
    if let Some(app_inode) = open_file(path.as_str(), OpenFlags::RDONLY) {
        let all_data = app_inode.read_all();
        let task = current_task().unwrap();
        let spawn = task.spawn(all_data.as_slice());
        let mut spawn_inner = spawn.inner_exclusive_access();
        let mut task_inner = task.inner_exclusive_access();
        spawn_inner.parent = Some(Arc::downgrade(&task));
        let pid = spawn.pid.0;
        let trap_cx = task_inner.get_trap_cx();
        task_inner.children.push(spawn.clone());
        trap_cx.x[10] = 0;
        drop(spawn_inner);
        drop(task_inner);
        add_task(spawn);
